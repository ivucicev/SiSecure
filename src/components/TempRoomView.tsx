...
export function TempRoomView({ mode, roomId, roomKeyB64, onExit }: TempRoomViewProps) {
  ...
  return (
    <motion.div
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      className="h-full overflow-hidden flex flex-col bg-obsidian-950 text-zinc-100"
    >
      {(phase === 'setup') && (
      ...
      {(phase === 'connecting' || phase === 'active') && (
        <>
          <div className="border-b border-zinc-800/60 p-5 pt-[max(1.25rem,env(safe-area-inset-top))] space-y-4">
